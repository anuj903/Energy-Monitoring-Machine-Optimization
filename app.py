...
def load_models_and_data():
    global model, le, forecast_model, data_stream, day_data, batch_data, test_results
    
    try:
        # Load model and encoder
        if os.path.exists('xgb_anomaly_model.pkl'):
        ...
        if os.path.exists('label_encoder.pkl'):
            ...
            logger.warning("Label encoder file not found")
            
        if os.path.exists('forecast_model.pkl'):
            forecast_model = joblib.load('forecast_model.pkl')
            logger.info("Successfully loaded forecast model")
        else:
        ...
        if os.path.exists(data_file):
            ...
            data_stream = pd.DataFrame()  # Create empty DataFrame

        # Load day level data
        day_data_file = '2_Melting_Day_level_data.csv'
        if os.path.exists(day_data_file):
            try:
            ...
            logger.warning(f"Day level data file not found: {day_data_file}")
            day_data = pd.DataFrame()  # Create empty DataFrame
        ...
        if os.path.exists(batch_data_file):
            ...
            logger.warning(f"Batch data file not found: {batch_data_file}")
            batch_data = pd.DataFrame()  # Create empty DataFrame
        ...
        if os.path.exists(test_results_file):
            try:
                ...
                if 'ds' in test_results.columns:
                    ...
                    logger.warning("Test results file must contain either 'ds' or 'timestamp' column")
            ...
            logger.warning(f"Test results file not found: {test_results_file}")
            test_results = pd.DataFrame()  # Create empty DataFrame
...
def stream_data():
    try:
        if data_stream is None or len(data_stream) == 0:
            return jsonify({
                'error': 'Data stream not available',
                'prediction': 'Error',
                'prescription': {
                    'possible_reason': 'Data not available',
                    'cause': 'Missing data file',
                    'solution': 'Please ensure data files are present'
                },
                'history': [],
        ...
        if model is None or le is None:
            label = 'normal'  # Default to normal if models not available
        else:
